...
class VisualPassCall {
    ...
    fun getVisualPass(context: Context, currentLocation: LatLng, callback: (VisualPassDataModel) -> Unit) {
        ...
        val call: Call<VisualPassDataModel> = service.getVisualPass(currentLocation.latitude, currentLocation.longitude, n2yoApiKey)
        ...
        call.enqueue(object : Callback<VisualPassDataModel> {
            ...
            override fun onResponse(response: Response<VisualPassDataModel>?, retrofit: Retrofit?) {
                ...
                if(response!!.isSuccess){
                    ...
                    Toast.makeText(context, response.body().info.toString(), Toast.LENGTH_LONG).show()
